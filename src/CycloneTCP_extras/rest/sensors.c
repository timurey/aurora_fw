...
register_rest_function (sensor, "/sensor", &restInitSensors, &restDenitSensors, &restGetSensors, &restPostSensors, &restPutSensors, &restDeleteSensors); // &restPostSensors, &restPutSensors, &restDeleteSensors);
...
static error_t forgotSensor(char * ID);
#if 0
...
error_t restPostSensors(HttpConnection *connection, RestApi_t* RestApi)
{
   ...
   if (error == NO_ERROR)
   {
      p+=snprintf(&path[0], arraysize(path), "/rest/v%"PRIi16"/sensors/", RestApi->restVersion);
   ...
   case ERROR_OUT_OF_MEMORY:
      connection->response.contentType = mimeGetType(".txt");
      return rest_500_int_server_error(connection, "500 Internal Server Error.\r\nOut of Memory\r\n");
      break;
   case ERROR_NOT_FOUND:
   default:
      connection->response.contentType = mimeGetType(".txt");
...
error_t restDeleteSensors(HttpConnection *connection, RestApi_t* RestApi)
{
   ...
   {
   case NO_ERROR:
      return rest_204_no_content(connection, &restBuffer[0]);
      break;
   case ERROR_NOT_FOUND:
   default:
      connection->response.contentType = mimeGetType(".txt");
...
IDListT * addDeviceId(const char * deviceId)
{
...
IDListT * sensorsFindDeviceId( char * deviceId)
{
...
static void forgotId(char *id)
{
   IDListT *ptr = NULL;

   ptr = sensorsFindDeviceId(id);
   if (ptr != NULL)
   {
      if (ptr->counter>0)
      {
      ...
      if (ptr->counter <=0)
      {
         list_remove(deviceIds, list_find(deviceIds, id));
      }
   }
}
...
static error_t forgotSensor(char * ID)
{
   ...
   {
      SensorListT * sensorPtr = sensListNode->val;
      currentSensor = &sensorPtr->sensor;

      forgotName(currentSensor->name);
      forgotPlace(currentSensor->place);
      forgotId(currentSensor->id);
      list_remove(sensors, sensListNode );
      error = NO_ERROR;
   }
   return error;
}
static error_t parseJSONSensors(jsmnParserStruct * jsonParser, configMode mode)
{
   ...
   uint8_t edit_mode = (mode) & (EDITMODE);
   uint8_t create_mode = (mode) & (CREATEMODE);
      ...
      if (error == NO_ERROR)
      {
         for (sensorNum = 0; sensorNum< (uint32_t) countOfSensors; sensorNum++)
         {
            ...
            if (result)
            {
               ...
               //If sensor exist and we want to create
               if ((currentSensor) && create_mode)
               {
                  error = ERROR_INVALID_RECIPIENT;
                  continue;
               }
               //If sensor not found and we want edit sensor
               if ((!currentSensor) && edit_mode) {
                  error = ERROR_NOT_FOUND;
                  continue;
               }
               //If sensor not found and we can create sensor
               if ((!currentSensor) && create_mode) {
