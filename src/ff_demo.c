...
// #endif	/* FF_USE_MKFS */
		...
		case 't' :	/* t [<year> <mon> <mday> <hour> <min> <sec>] - Set/Show RTC */
			...
			// 	xatoi(&ptr, &p1); rtc.day = (BYTE)p1;
			// 	xatoi(&ptr, &p1); rtc.hours = (BYTE)p1;
			// 	xatoi(&ptr, &p1); rtc.minutes = (BYTE)p1;
 			// 	if (!xatoi(&ptr, &p1)) break;
			// 	rtc.sec = (BYTE)p1;
			// 	rtc_settime(&rtc);
			// }
			getCurrentDate(&rtc);
			xprintf("%u/%u/%u %s %02u:%02u:%02u\n", rtc.year, rtc.month, rtc.day, &days[rtc.dayOfWeek*4], rtc.hours, rtc.minutes, rtc.seconds);
			break;
